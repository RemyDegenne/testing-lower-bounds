...
Goal: bounds on $B_\pi(\mu^{\otimes n}, \nu^{\otimes n})$ or on $\inf_{E} \max\{\mu^{\otimes n}(E), \nu^{\otimes n}(E^c)\}$ that show how it depends on $n$.

Idea: relate these to divergences that tensorize, or on which we have tensorization inequalities. Those are $\KL$, $R_\alpha$, $\JS_\alpha$ (and more generally the mutual information if we want to consider more than two hypotheses).
...
\begin{lemma}[Fano's inequality, binary case]

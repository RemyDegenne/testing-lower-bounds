...
\: .
\end{align*}
The second inequality uses $f \ge 0$ and the last inequality uses $f \le 1$~.
\end{proof}
...
We apply Lemma~\ref{lem:change_measure_risk} to $\zeta \times \xi$ and $\pi \otimes P$ and remark that by Lemma~\ref{lem:bayesRisk_const} the risk for a constant kernel with value $\xi$ is independent of $\xi$ and equal to $\mathcal R_\zeta^{d_{\mathcal X}}$.
\end{proof}

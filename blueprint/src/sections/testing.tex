...
Let $u \in \mathcal P(\{0,1\})$ be the probability distribution with $u_0 = 1/2$. We write $\pi_\alpha$ for the probability measure on $\{0,1\}$ with $\pi_0 = \alpha$. By Theorem~\ref{thm:kl_compProd} and Lemma~\ref{lem:jensenShannon_eq_kl},
\begin{align*}
...
\begin{align*}
\KL(\ell \circ (\mathrm{id} \times \hat{y}_B) \circ (\pi_\alpha \otimes P), \ell \circ (\mathrm{id} \times \hat{y}_B) \circ (u \times (P^{\otimes n} \circ \pi_\alpha)))
\le \KL(\pi_\alpha \otimes P, u \times (P \circ \pi_\alpha))
\: .
\end{align*}
The two measures on the left are measures on $\{0,1\}$, with $\ell \circ (\mathrm{id} \times \hat{y}_B) \circ (u \times (P \circ \pi_\alpha)) = u$ and $(\ell \circ (\mathrm{id} \times \hat{y}_B) \circ (\pi_\alpha \otimes P)) (\{1\}) = B_\alpha(\mu, \nu)$.
We get
\begin{align*}
\KL(\pi_\alpha \otimes P, u \times (P \circ \pi_\alpha))
\ge \kl(B_\alpha(\mu, \nu), 1/2)

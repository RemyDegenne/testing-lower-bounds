...
The next results are adapted from \cite{pensia2024sample}.
...
\begin{lemma}
  ...
  The sample complexity of simple binary hypothesis testing satisfies
  \begin{align*}
...
\begin{proof}%\leanok
\uses{thm:kl_data_proc, thm:kl_compProd, lem:jensenShannon_prod_n, lem:jensenShannon_eq_kl}
TODO: split lemmas out of this proof.

Let $u \in \mathcal P(\{0,1\})$ be the probability distribution with $u_0 = 1/2$. We write $\xi_\pi$ for the probability measure on $\{0,1\}$ with $\xi_0 = \pi$. By Theorem~\ref{thm:kl_compProd} and Lemma~\ref{lem:jensenShannon_eq_kl},
\begin{align*}
\KL(\xi_\pi \otimes P^{\otimes n}, u \times (P^{\otimes n} \circ \xi_\pi))
&= \kl(\pi, 1/2) + \KL(\xi_\pi \otimes P^{\otimes n}, \xi_\pi \times (P^{\otimes n} \circ \xi_\pi))
\\
&= \kl(\pi, 1/2) + \JS_\pi(\mu^{\otimes n}, \nu^{\otimes n})
\\
&= \log 2 - h_2(\pi) + \JS_\pi(\mu^{\otimes n}, \nu^{\otimes n})
\: .
\end{align*}

Let $\hat{y}_B$ be the generalized Bayes estimator. By the data-processing inequality for $\KL$ (Theorem~\ref{thm:kl_data_proc}),
\begin{align*}
\KL(\ell' \circ (\mathrm{id} \times \hat{y}_B) \circ (\xi_\pi \otimes P^{\otimes n}), \ell' \circ (\mathrm{id} \times \hat{y}_B) \circ (u \times (P^{\otimes n} \circ \xi_\pi)))
\le \KL(\xi_\pi \otimes P^{\otimes n}, u \times (P^{\otimes n} \circ \xi_\pi))
\: .
\end{align*}
The two measures on the left are measures on $\{0,1\}$, with $\ell' \circ (\mathrm{id} \times \hat{y}_B) \circ (u \times (P^{\otimes n} \circ \xi_\pi)) = u$ and $(\ell' \circ (\mathrm{id} \times \hat{y}_B) \circ (\xi_\pi \otimes P^{\otimes n})) (\{1\}) = B_\pi(\mu^{\otimes n}, \nu^{\otimes n})$.
We get
\begin{align*}
\KL(\xi_\pi \otimes P^{\otimes n}, u \times (P^{\otimes n} \circ \xi_\pi))
\ge \kl(B_\pi(\mu^{\otimes n}, \nu^{\otimes n}), 1/2)
= \log 2 - h_2(B_\pi(\mu^{\otimes n}, \nu^{\otimes n}))
\: .
\end{align*}
We proved
\begin{align*}

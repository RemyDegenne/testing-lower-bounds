...
\begin{lemma}
  \label{lem:fDiv_map_eq_fDiv_trim}
  %\lean{}
  %\leanok
  \uses{}
  Let $\mu, \nu \in \mathcal M(\mathcal X)$ be finite measures and let $g : \mathcal X \to \mathcal Y$ be a measurable function. Denote by $g^* \mathcal Y$ the comap of the $\sigma$-algebra on $\mathcal Y$ by $g$. Then
...
\begin{proof}%\leanok
\uses{thm:fDiv_trim_le, lem:fDiv_map_eq_fDiv_trim}
By Lemma~\ref{lem:fDiv_map_eq_fDiv_trim}, $D_f(g_* \mu, g_* \nu) = D_f(\mu_{| g^* \mathcal Y}, \nu_{| g^* \mathcal Y})$. Then apply Theorem~\ref{thm:fDiv_trim_le}.
\end{proof}

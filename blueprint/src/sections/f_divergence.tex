...
TODO: move this subsection earlier?
It will be used to obtain the most general data-processing inequality.
...
\begin{proof}%\leanok
\uses{lem:convex_taylor_lt, lem:convex_taylor_gt}
...
\begin{definition}
  ...
  %\leanok
  \uses{}
  For $\gamma \in (0, +\infty)$ let $\phi_\gamma : \mathbb{R} \to \mathbb{R}$ be the function defined by
  \begin{align*}
  \phi_\gamma(x) &= \max\left\{0, x - \gamma \right\} & \text{ for } \gamma \ge 1 \: ,
  \\
  \phi_\gamma(x) &= \max\left\{0, \gamma - x \right\} & \text{ for } \gamma < 1 \: .
  \end{align*}
\end{definition}

\begin{corollary}
  \label{cor:convex_taylor_eGammaFun}
  %\lean{}
  %\leanok
  \uses{def:curvatureMeasure, def:eGammaFun}
  For $f: \mathbb{R} \to \mathbb{R}$ a convex function with $f(1) = 0$ and $f'_+(1) = 0$~, for all $x \in \mathbb{R}$~,
  \begin{align*}
  f(x) &= \int_{y} \phi_y(x) \partial\gamma_f \: .
  \end{align*}
\end{corollary}

\begin{proof}%\leanok
\uses{cor:convex_taylor_one}
...
= \int_{y \in (1, +\infty)} \phi_y(x) \partial\gamma_f
\: .
\end{align*}
That final integral is equal to $\int_y \phi_y(x) \partial\gamma_f$ (without the restriction to $(1, +\infty)$) because $\phi_y(x)$ is zero outside of $(1, +\infty)$ for $x \ge 1$.

The case of $x < 1$ is similar.
\end{proof}

\begin{lemma}
  \label{lem:curvatureMeasure_eGammaFun}
  %\lean{}
  %\leanok
  \uses{def:curvatureMeasure, def:eGammaFun}
  The curvature measure of the function $\phi_\gamma$ is $\gamma_{\phi_\gamma} = \delta_\gamma$~, the Dirac measure at $\gamma$.
\end{lemma}
...
TODO: The $E_\gamma$ divergence is not defined yet. We need to move some things around.
...
From Corollary~\ref{cor:convex_taylor_eGammaFun} we get that $f(x) = \int_y \phi_y(x) \partial\gamma_f$~.
From this and the theorem of Fubini,
\begin{align*}
\int_x f\left( \frac{d \mu}{d \nu}(x) \right) \partial\nu
= \int_{x} \int_{y} \phi_y \left(\frac{d \mu}{d \nu}(x) \right) \partial\gamma_f \partial\nu
= \int_{y} \int_{x} \phi_y \left(\frac{d \mu}{d \nu}(x) \right) \partial\nu \partial\gamma_f
\: .
\end{align*}
The inner integral is a divergence: $\int_{x} \phi_y \left(\frac{d \mu}{d \nu}(x) \right) \partial\nu = E_y(\mu, \nu)$~.
\end{proof}
...
\begin{lemma}
  \label{lem:curvatureMeasure_deGrootFun}
  %\lean{}
  %\leanok
  \uses{def:curvatureMeasure, def:deGrootFun}
  The curvature measure of the function $\psi_\pi$ is $\gamma_{\psi_\pi} = \pi\delta_{(1 - \pi)/\pi}$~.
\end{lemma}

\begin{proof}%\leanok
\uses{lem:curvatureMeasure_eGammaFun, lem:curvatureMeasure_mul}

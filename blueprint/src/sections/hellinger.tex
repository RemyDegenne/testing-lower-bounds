...
\begin{definition}[Squared Hellinger distance]
  ...
  %\leanok
  \uses{def:fDiv}
  Let $\mu, \nu$ be two measures on $\mathcal X$. The squared Hellinger distance between $\mu$ and $\nu$ is
  \begin{align*}
...
\begin{lemma}[Hellinger and Rényi]
  ...
  Let $\mu, \nu$ be two probability measures on $\mathcal X$. Then $R_{1/2}(\mu, \nu) = -2\log(1 - H^2(\mu, \nu))$.
\end{lemma}
...
\begin{lemma}
  ...
  Let $\mu, \nu$ be two probability measures on $\mathcal X$. Then $H^2(\mu, \nu) \le TV(\mu, \nu)$.
\end{lemma}

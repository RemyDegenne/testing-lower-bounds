...
\begin{lemma}
  ...
  %\lean{}
  %\leanok
  \uses{def:fMutualInfoLeft, def:Renyi}
...
\begin{lemma}
  ...
  %\lean{}
  %\leanok
  \uses{def:fMutualInfoRight, def:mutualInfo}
...
\begin{corollary}
  ...
  %\lean{}
  %\leanok
  \uses{def:fMutualInfoRight, def:jensenShannon}
  For $\mu \in \mathcal P(\{0,1\})$, $\kappa : \{0,1\} \rightsquigarrow \mathcal Y$ and $f : x \mapsto x \log x$ (such that $D_f = \KL$),
  \begin{align*}
...
\begin{proof}%\leanok
\uses{lem:fMutualInfoRight_eq_mutualInfo, lem:mutualInfo_eq_jensenShannon}

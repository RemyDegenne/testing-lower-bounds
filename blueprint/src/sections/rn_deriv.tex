...
\begin{lemma}
  ...
  %\leanok
  %\uses{}
  Let $\mu$ be a measure on $\mathcal X$ and let $\kappa, \eta : \mathcal X \rightsquigarrow \mathcal Y$ be two finite kernels with $\kappa(x) \ll \eta(x)$ for $\mu$-almost all $x$. Then for $(\mu \otimes \eta)$-almost all $(x, y)$, $\frac{d (\mu \otimes \kappa)}{d (\mu \otimes \eta)}(x,y) = \frac{d \kappa}{d \eta}(x,y)$.
  This implies that the equality is true for $\mu$-almost all $x$, for $\eta(x)$-almost all $y$.
\end{lemma}

\begin{proof}
\uses{cor:rnDeriv_value}
It is sufficient to prove that the integrals of the two functions on all product set of the shape $s \times t$ for $s$ and $t$ measurable are equal (since these sets are a $\pi$-system). The first one is a direct computation:
\begin{align*}
\int_{p \in s \times t}\frac{d (\mu \otimes \kappa)}{d (\mu \otimes \eta)}(p) \partial(\mu \otimes \eta)
&= (\mu \otimes \kappa)(s \times t)
\: .
\end{align*}
The second one uses Corollary~\ref{cor:rnDeriv_value}.
\begin{align*}
\int_{p \in s \times t}\frac{d \kappa}{d \eta}(p) \partial(\mu \otimes \eta)
&= \int_{x \in s} \int_{y \in t} \frac{d \kappa}{d \eta}(x,y) \partial\eta(x) \partial\mu
\\
&= \int_{x \in s} \int_{y \in t} \frac{d \kappa(x)}{d \eta(x)}(y) \partial\eta(x) \partial\mu
\\

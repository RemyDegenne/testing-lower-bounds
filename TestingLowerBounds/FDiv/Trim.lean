...
See `fDiv_comp_le_of_ac` for the more general statement about composition by a kernel. -/
...
lemma fDiv_map_le_of_ac [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (hμν : μ ≪ ν) {g : α → β} (hg : Measurable g) (hf : StronglyMeasurable f)
    (hf_cvx : ConvexOn ℝ (Ici 0) f) (hf_cont : ContinuousOn f (Ici 0)) :
    fDiv f (μ.map g) (ν.map g) ≤ fDiv f μ ν := by
...
theorem fDiv_map_le [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    ...
    (hf_cvx : ConvexOn ℝ (Ici 0) f) (hf_cont : ContinuousOn f (Ici 0)) :
    fDiv f (μ.map g) (ν.map g) ≤ fDiv f μ ν := by
  have h1 : μ.map g = (ν.withDensity (∂μ/∂ν)).map g + (μ.singularPart ν).map g := by
    conv_lhs => rw [μ.haveLebesgueDecomposition_add ν, add_comm, Measure.map_add _ _ hg]
  rw [h1, fDiv_eq_add_withDensity_derivAtTop μ ν hf_cvx]
  refine (fDiv_add_measure_le _ _ _ hf hf_cvx).trans ?_
  refine add_le_add ?_ ?_
  · exact fDiv_map_le_of_ac (withDensity_absolutelyContinuous ν (∂μ/∂ν)) hg hf hf_cvx hf_cont
  · rw [Measure.map_apply hg MeasurableSet.univ, preimage_univ]

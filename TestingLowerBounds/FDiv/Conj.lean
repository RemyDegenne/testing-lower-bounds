...
lemma fDiv_conj (μ ν : Measure α) [SigmaFinite μ] [SigmaFinite ν] :
  ...
  have h_left : ∫⁻ x, f.conj ((∂μ/∂ν) x) ∂ν
    ...
    rw [lintegral_eq_add_singularPart_withDensity μ ν measurable_divFunction_rnDeriv]
    congr 1
  ...
  have h_right : ∫⁻ x, f ((∂ν/∂μ) x) ∂μ
    ...
    rw [lintegral_eq_add_singularPart_withDensity ν μ measurable_divFunction_rnDeriv]
    congr 1
  ...
  have h_key : ∫⁻ x, (∂ν/∂μ) x * f.conj ((∂μ/∂ν) x) ∂μ
    ...
    rw [lintegral_eq_add_singularPart_withDensity ν μ (g := fun x ↦ (∂ν/∂μ) x * f.conj ((∂μ/∂ν) x))
...
private lemma coe_mul_add_ne_zero (hab : a + b = 1) {x : ℝ≥0∞} (hx0 : x ≠ 0) :
    (a : ℝ≥0∞) * x + b ≠ 0 := by
  intro h
...
lemma DivFunction.conj_compAffine_apply (hab : a + b = 1) {x : ℝ≥0∞} (hx0 : x ≠ 0) :
    f.conj.compAffine a b hab x = (a * x + b) * f (a * x + b)⁻¹ := by
  rw [DivFunction.compAffine_apply, DivFunction.conj_of_ne_zero (coe_mul_add_ne_zero hab hx0)]

/-- Value of the divergence function of `fDiv_smul_add_smul_right` away from `0` and `∞`. -/
lemma DivFunction.conj_compAffine_conj_apply (hab : a + b = 1) {x : ℝ≥0∞} (hx0 : x ≠ 0)
    (hx : x ≠ ∞) :
  ...
  have h0 : (b : ℝ≥0∞) * x⁻¹ + a ≠ 0 := by
    rw [h_eq]
    exact mul_ne_zero hx_inv (coe_mul_add_ne_zero hab hx0)

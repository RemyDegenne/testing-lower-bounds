...
lemma Measure.comp_eq_snd_compProd (μ : Measure α) [SFinite μ]
    ...
    κ ∘ₘ μ = (μ ⊗ₘ κ).snd := by
  ext s hs
  rw [Measure.bind_apply hs κ.aemeasurable, Measure.snd_apply hs, Measure.compProd_apply]
  · rfl
...
lemma Measure.fst_swap_compProd [SFinite μ] [IsSFiniteKernel κ] :
  ...
  simp [Measure.comp_eq_snd_compProd, -MeasureTheory.Measure.snd_compProd]
...
lemma Measure.comp_apply_univ [IsMarkovKernel κ] : (κ ∘ₘ μ) .univ = μ .univ := by
  rw [Measure.bind_apply .univ κ.aemeasurable]
  simp
...
lemma Measure.comp_smul_left (a : ℝ≥0∞) : κ ∘ₘ (a • μ) = a • (κ ∘ₘ μ) := by
  ext s hs
  simp only [Measure.bind_apply hs κ.aemeasurable, lintegral_smul_measure, Measure.smul_apply,
    smul_eq_mul]
...
lemma Measure.absolutelyContinuous_comp {μ ν : Measure α} {κ η : Kernel α γ}
  ...
  exact Measure.AbsolutelyContinuous.map (Measure.AbsolutelyContinuous.compProd hμν hκη)
    measurable_snd

...
instance IsMarkovKernel.fst' (κ : kernel (α × β)  γ) (b : β) [IsMarkovKernel κ] :
    IsMarkovKernel (fst' κ b) := by
  rw [kernel.fst']; infer_instance

instance IsFiniteKernel.fst' (κ : kernel (α × β)  γ) (b : β) [IsFiniteKernel κ] :
    IsFiniteKernel (fst' κ b) := by
  rw [kernel.fst']; infer_instance

instance IsSFiniteKernel.fst' (κ : kernel (α × β)  γ) (b : β) [IsSFiniteKernel κ] :
    IsSFiniteKernel (fst' κ b) := by
  rw [kernel.fst']; infer_instance

instance (priority := 100) isMarkovKernel_of_isMarkovKernel_fst' {κ : kernel (α × β)  γ}
    [h : ∀ b, IsMarkovKernel (fst' κ b)] :
    IsMarkovKernel κ := by
  refine ⟨fun ab ↦ ⟨?_⟩⟩
  rw [← fst'_apply]
...
/-- Define a `kernel β γ` from a `kernel (α × β) γ` by taking the comap of TODO:what is this function called?. -/
...
instance IsMarkovKernel.snd' (κ : kernel (α × β)  γ) (a : α) [IsMarkovKernel κ] :
    IsMarkovKernel (snd' κ a) := by
  rw [kernel.snd']; infer_instance

instance IsFiniteKernel.snd' (κ : kernel (α × β)  γ) (a : α) [IsFiniteKernel κ] :
    IsFiniteKernel (snd' κ a) := by
  rw [kernel.snd']; infer_instance

instance IsSFiniteKernel.snd' (κ : kernel (α × β)  γ) (a : α) [IsSFiniteKernel κ] :
    IsSFiniteKernel (snd' κ a) := by
  rw [kernel.snd']; infer_instance

instance (priority := 100) isMarkovKernel_of_isMarkovKernel_snd' {κ : kernel (α × β)  γ}
    [h : ∀ b, IsMarkovKernel (snd' κ b)] :
    IsMarkovKernel κ := by
  refine ⟨fun ab ↦ ⟨?_⟩⟩
  rw [← snd'_apply]
  simp only [measure_univ]

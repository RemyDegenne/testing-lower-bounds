-- This file is copied from the PFR project. Author: Rémy Degenne
...
noncomputable
def deleteMiddle (κ : kernel α (β × γ × δ)) : kernel α (β × δ) :=
...
@[simp]
lemma reverse_reverse (κ : kernel α (β × γ × δ)) : reverse (reverse κ) = κ := by
